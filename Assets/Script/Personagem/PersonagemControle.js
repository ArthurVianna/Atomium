...
var jump : AudioClip;
private var respawnarbool :boolean;
...
function Start () {
	...
	vel = 0;
	vida = 3;
	if(PlayerPrefs.GetInt("torrebool") == 1){
	...
	if(PlayerPrefs.GetInt("torre"+"1")==1||PlayerPrefs.GetInt("torre"+"2")==1){
	this.gameObject.transform.position.x = PlayerPrefs.GetInt("x");
	this.gameObject.transform.position.y =PlayerPrefs.GetInt("y");
	vida = PlayerPrefs.GetInt("vida");
	}else{
...
function Update () {
			...
			if (Input.GetKeyDown("w") && puloDuplo <2 || Input.GetKeyDown("up") && puloDuplo <2){
